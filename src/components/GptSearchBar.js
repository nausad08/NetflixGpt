...
import lang from '../utils/langugeConstant';
...
const GptSearchBar = () => {
  ...
  const handleGptSearchClick = async() =>{
    ...
    // make an api ccall to gpt api and grt movie result
    ...
    const gptMovies = gptResults.choices[0]?.message?.content.split(",") // this  split gives an array 


    // for each movie i will search tmdb api
    const promishArray = gptMovies.map((movie)=>searchMovieTMDB(movie));
    //[promish,promish,promish,promish, promish]
    const tmdbResult = await Promise.all(promishArray);
